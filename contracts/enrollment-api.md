...
`ai-telemetry-pipeline`의 `sql/rds/seed.sql`은 `HMAC-SHA256('dev-only-invite-pepper', code)`를 전제하지만
backend는 무염 SHA-256이다. **시드는 dev 편의용이고 진실원은 backend다.** 시드를 backend 방식에 맞춘다.
...
| # | 항목 | 영향 |
|---|---|---|
| M1 | **manifest endpoint 시드 불일치** — backend `LocalSeeder`는 `:4318`(collector 직결, auth-proxy 우회·무인증·무귀속이자 데몬 로컬 수신기 기본 포트와 자기참조 위험), 파이프라인 시드는 `:4316`(정상 경로) | 로컬 구동 시 신호가 인증을 건너뛰거나 자기참조 루프 |
...
| M9 | manifest `protocol: "grpc"`는 서버 검증을 통과하지만 클라이언트가 상위 전송을 지원하지 않아 데몬이 기동을 거부한다 | 서버에서 grpc를 막거나 클라이언트에 구현해야 한다 |
...
| — | `contracts/enrollment-manifest.schema.json`의 `privacy.collect_raw_api_bodies`만 `required` 누락 | Go 구조체와 비대칭 |
